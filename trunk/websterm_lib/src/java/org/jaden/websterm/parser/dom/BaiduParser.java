 ...
 * @author Jack Shen
 */
public class BaiduParser extends SearchEngineDOMParser {
	...
	public List<SearchResult> parseDocument() {
		...
		NodeList tds = body.getElementsByTagName("td");
		for (int i = 0; i < tds.getLength(); i++) {
			Element td = (Element) tds.item(i);
			if (td.getAttribute("class").equalsIgnoreCase("f")) {
				Element link = (Element) td.getElementsByTagName("a").item(0);
				SearchResult result = new SearchResult();
				result.setUrl(link.getAttribute("href"));
